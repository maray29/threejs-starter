...
export class Debug {
  ...
  #createLightConfig() {
    ...
    const folder2 = this.pane.addFolder({ title: "Ambient Light" });

    this.#createColorControl(this.app.ambientLight, folder2);

    folder2.addInput(this.app.ambientLight, "intensity", {
      label: "Intensity",
      min: 0,
      max: 10,
    });

    const folder3 = this.pane.addFolder({ title: "Directional Light" });

    this.#createColorControl(this.app.ambientLight, folder3);

    folder3.addInput(this.app.directionalLight, "intensity", {
      label: "Intensity",
      min: 0,
      max: 10,
    });
  ...
  #createColorUniformControl(obj, folder, uniformName, label = "Color") {
    ...
    const baseColor255 = obj.material.uniforms[uniformName].value
      .clone()
      .multiply(preMultVector);
    const params = {
      color: {
        r: baseColor255.x,
        g: baseColor255.y,
